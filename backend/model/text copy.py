...
def chunk(text, chunk_option):
    ...
    def process_chunks(chunks_to_process):
        ...
        avg_chunk_size = len(text) / max(len(chunks_to_process), 1) 
        ...
        for chunk in chunks_to_process:
            ...
            threshold = (target_chunk_size / avg_chunk_size) * chunk_length
            # print(f"avg_chunk_size {avg_chunk_size} \nchunk_length {chunk_length} \nthreshold {threshold} \n")

            # Determines if length of current chunk exceeds threshold
            if total_chars + chunk_length > threshold and total_chars > 0:
...
# chunked = chunk(input_text, 'sentences')
