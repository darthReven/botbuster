...
# Defining the hyperparameters (optimizer, weights of the classes, epochs, and L1 strength)
optimizer = torch.optim.AdamW(model.parameters(), lr=1e-5, weight_decay=0.00001)  # Added L1 regularization (weight_decay)
cross_entropy = nn.NLLLoss()
epochs = 2
l1_strength = 0.00001  # Adjust the L1 strength as needed

# Training and Evaluation Functions
def l1_regularization(model, l1_strength):
    l1_reg = torch.tensor(0.0, requires_grad=True).to(device)
    for param in model.parameters():
        l1_reg = l1_reg + torch.norm(param, p=1)
    return l1_strength * l1_reg
...
def train():
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        cross_entropy_loss = cross_entropy(preds, torch.LongTensor(labels).to(device))
        
        # L1 regularization
        l1_loss = l1_regularization(model, l1_strength)
...
for epoch in range(epochs):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model.state_dict(), 'new_model_weights3.pt')
...
model.load_state_dict(torch.load('new_model_weights3.pt'))
